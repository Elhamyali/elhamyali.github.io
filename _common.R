...
# Use the local CSV file for publications
pubs <- read_csv("publications/pubs.csv")  # Make sure the path is correct
...
make_media_list <- function() {
  # Read the local CSV file for media data
  media <- read_csv("publications/media.csv")  # Ensure the correct path to your local file
  temp <- media %>%
    mutate(
